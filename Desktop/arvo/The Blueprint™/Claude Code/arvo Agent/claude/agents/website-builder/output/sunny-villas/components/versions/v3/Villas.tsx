...
function VillaRow({ villa, index }: { villa: typeof villas[0]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className="grid md:grid-cols-[65fr_35fr] gap-0 items-stretch"
    ...
    >
      ...
      >
        ...
        <h3 className="font-display text-[28px] md:text-[36px] text-navy leading-tight uppercase mb-3">
          {villa.name}
        </h3>
        <p className="text-[12px] text-rust/70 tracking-[0.06em] mb-6">
          {villa.feature}
        </p>
        <p className="text-[13px] leading-[1.85] text-muted max-w-md mb-8">
          {villa.description}
        </p>

        <div className="flex flex-wrap items-center gap-4 text-[11px] text-muted mb-8">
...
export default function Villas() {
  ...
  return (
    <section id="villas" ref={ref}>
      ...
      {/* Villa rows - alternating image/text, 65/35 split */}
      {villas.map((villa, i) => (
